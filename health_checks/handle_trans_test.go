...
func (r *MockCFlare) GetAddrs() []string {
	return []string{"52.52.52.52"}
}
...
func mockInitMaster(dnsConfig dns.Provider, threshold int) {
...
func Test_handleTransition(t *testing.T) {
	Convey("When running fastping", t, func() {
		...
		cf, _ := mockConfigureCloudflare()
		dnsConfig = cf
		InitGM(dnsConfig.GetAddrs(), true)
		Gm.Clustered = false
